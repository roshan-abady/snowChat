create or replace view ES_ERP_CONTRACT_BY_PRODUCT_ITEM_EDITION(
	MONTH_ID,
	PRODUCT_SEGMENT,
	CUSTOMER_ID,
	ARCHIE_CLIENT_ID,
...
--CHURN_MONTH_FLAG=1
,CASE WHEN NEW_LOGO_FLAG=0 AND (CHURN_MONTH_ID<>MONTH_ID OR CHURN_MONTH_ID IS NULL) THEN ROUND(SUM(coalesce(PRICE_CHANGE,0)),2) ELSE 0 END AS ARR_PRICE_CHANGE
...
,CASE WHEN NEW_LOGO_FLAG=0 AND (CHURN_MONTH_ID<>MONTH_ID OR CHURN_MONTH_ID IS NULL) THEN SUM(CLOSE_ARR - OPEN_ARR - coalesce(PRICE_CHANGE,0) - ROUND(DISCOUNT_EXPIRY_ARR,2))
...
SELECT
    *,MAX(coalesce(cast(REVISION_NUMBER_END_MONTH as integer),0)) OVER (PARTITION BY CONTRACT_NUM,MONTH_ID) AS REVISION_NUMBER_END_MONTH_1
     FROM (
     OPERATIONS_ANALYTICS.TRANSFORMED_PROD.ES_ERP_CONTRACT_BY_PRODUCT_TABLE-- was withouth _TABLE --es_contract_line_by_contract_item_monthly_view
    --
        ) EDITION
--
--     WHERE
...
FSWD as (
 SELECT DISTINCT
  ...
  FROM CRM.PUBLISHED_PROD.OPPORTUNITY AS OPTY
  LEFT JOIN CRM.PUBLISHED_PROD.ACCOUNT AS ACC ON ACC.ACCOUNT_ID = OPTY.ACCOUNTID
